 ...
 */
export function extractRootFunctionsWithThrows(
  sourceCode: string,
): FunctionInfo[] {
  ...
  function visit(
    ...
    if (ts.isFunctionDeclaration(node) && node.name) {
      ...
      ts.forEachChild(
        node,
        (child) => visit(child, newParentChain, nestLevel + 1),
      );
    } else {
      // 関数以外のノードも探索
...
if (import.meta.main) {
  ...
  if (args.length === 0) {
    console.error(
      "使用法: deno run --allow-read extract-functions.ts <ファイルパス>",
    );
    Deno.exit(1);
  }
        ...
        if (func.throws.length > 0) {
          ...
          func.throws.forEach((throwInfo) => {
            if (throwInfo.description) {
              console.log(
                `      - ${throwInfo.type}: ${throwInfo.description}`,
              );
            } else {
